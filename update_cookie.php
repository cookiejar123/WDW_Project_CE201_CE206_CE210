...
if (!isset($_SESSION['userid']) || !isset($_GET['update'])) {
...
<body>
    ...
    <div class="container mt-3">
        ...
        <?php
        $query = "SELECT * FROM `cookie` where cid = '" . $_GET['update'] . "' and uid = '" . $_SESSION['userid'] . "' limit 1";
        ...
        <div class="card">
            <h5 class="card-header">Update Cookie</h5>
            <div class="card-body">
                <form action="./partial/update_cookie.php" method="POST">
                    <input type="hidden" name="cid" value="<?php echo $_GET['update'] ?>">
                    <div class="form-floating mb-3">
                        <input type="text" class="form-control" id="c_title" name="c_title" placeholder="Its easy, i can do it." value="<?php echo $row['title'] ?>" required>
                        <label for="floatingInput">Enter Cookie Title</label>
                    </div>
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" id="c_isPublic" name="c_isPublic" <?php if ($row['is_public'] == 1) echo "checked" ?>>
                        <label class="form-check-label" for="c_isPublic">Check if you want to make it public.</label>
                    </div>

                    <?php
                    $query1 = "SELECT * FROM `cookie_msg` where cid = '" . $_GET['update'] . "' limit 1";
                    $result1 = mysqli_query($conn, $query1);
                    $isMsg = mysqli_num_rows($result1);
                    $row1 = mysqli_fetch_assoc($result1);
                    ?>

                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input <?php if ($isMsg != 1) echo "collapsed" ?>" type="checkbox" onclick="enableMsg(this)" id="c_isMsg" name="c_isMsg" data-bs-toggle="collapse" data-bs-target="#collapseMsg" aria-expanded="<?php if ($isMsg == 1) echo "true"; else echo "false"; ?>" aria-controls="collapseMsg" <?php if ($isMsg == 1) echo "checked" ?>>
                        <label class="form-check-label" for="c_isMsg">Check/uncheck if you want to add/delete description for cookie.</label>
                    </div>
                    <div class="form-floating mb-3 <?php if ($isMsg == 1) echo " collapse show"; else echo " collapse "; ?>" id="collapseMsg">
                        <textarea class="form-control" placeholder="Leave a comment here" id="c_msg" name="c_msg" style="height: 100px"> <?php if ($isMsg == 1) echo $row1['msg'] ?></textarea>
                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <div class="row" id="c_tags">
                                <?php
                                $query2 = "select cid,ct.tid,uid,name from cookie_tag ct, tags tg where ct.tid = tg.tid and cid = '".$_GET['update']."'";
                                $result2 = mysqli_query($conn, $query2);
                                while($row2 = mysqli_fetch_assoc($result2)){
                                    echo '<div class="alert alert-dark alert-dismissible fade show col-12 new_tag" role="alert">
                                    '.$row2['name'].'
                                    <input type="hidden" name="c_tags[]" value="'.$row2['tid'].'">
                    ...
                    <button type="submit" class="btn btn-outline-danger">Update it</button>
